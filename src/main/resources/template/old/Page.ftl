<div class="col-xs-12 nopd-lr wigtbox-head form-inline">
    <form id="search_form" action="/app/${table_name_first_low}/grid" method="Post">
        <div class="pull-left">
        ...
        <div class="pull-right">
            ...
            <button class="btn btn-danger btn-sm" type="button"  onclick=confrimDel2("#grid-table","/app/${table_name_first_low}/delete")>
                        ...
                        <#list model_column as model>
                            <div class="form-group">
                                <div class="col-md-12">
                                    ...
                                    <div class="${model.columnName} col-md-7 nopd-lr"></div>
                                </div>
                            </div>
                        </#list>
                    </#if>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <form id="info_form" method="Post" class="form-horizontal" action="/app/${table_name_first_low}/addOrEdit">
                    <div class="form-group">
                        <#if model_column?exists>
                            <#list model_column as model>
                                <div class="col-md-12">
                                    <label for="${model.columnName}" class="col-md-5">${model.columnComment!}</label>
                                    <div class="col-md-7 nopd-l"><input type="text" class="form-control" name="${model.columnName}" id="${model.columnName}" placeholder=""/></div>
                                </div>
                            </#list>
                        </#if>
                    </div>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $(grid_selector).jqGrid({
            url:"/app/${table_name_first_low}/grid",
            mtype: "post",
            datatype: "json",
            height: '100%',
            colNames:[
                ...
                {name:'myac',index:'', fixed:true, sortable:false, resize:false,
                    formatter: function(value,grid,rows,state){
                        return '<a class="btn btn-info btn-xs mr5" data-toggle="modal" data-target="#detail-dialog" data-id="'+rows.ID+'" title="查看详情"><i class="fa fa-eye"></i></a>'+
                                ...
                                '<a class="btn btn-danger btn-xs mr5" data-toggle="tooltip" data-placement="top" title="删除" onclick=confrimDel2("#grid-table","/app/${table_name_first_low}/delete")><i class="fa fa-trash"></i></a>';
                    }
                }
            ],
            viewrecords : true,
        ...
        $('#detail-dialog').on('show.bs.modal',function(e){
            ...
            //发送ajax回去查询单条数据
            $.ajax({
                url:'/app/${table_name_first_low}/detail',
                type:'Get',
                data:{id:id},
                success:function(result){
                    if(result){
                        ...
                        <#if model_column?exists>
                            <#list model_column as model>
                                modal.find('.${model.columnName}').text(result.${model.changeColumnName?uncap_first});
                            </#list>
                        </#if>
                    }
                    else
                ...
                $.ajax({
                    ...
                    success:function(result){
                        if(result){
                            ...
                            <#if model_column?exists>
                                <#list model_column as model>
                                modal.find('#${model.columnName}').val(result.${model.changeColumnName?uncap_first});
                                </#list>
                            </#if>
                        }
                        else
        ...
        $('#btn-confirm').on('click',function(){
            ...
            if($('#info_form').valid()){
                $.ajax({
                    url:'/app/${table_name_first_low}/addOrEdit',
                    type:'Post',
                    dataType:'json',
    ...
    function exportExcelData(){
        ...
        <#if model_column?exists>
            ...
            var path = '/app/exportDate/${table_name_first_low}/export?'
            <#list model_column as model>
